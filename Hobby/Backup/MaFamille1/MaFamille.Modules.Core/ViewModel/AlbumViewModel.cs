...
{    
    public class AlbumViewModel:NotificationObject//, INotifyPropertyChanged
    {
        ...
        public AlbumViewModel()
        {
            ...
            {
                //_Context.Load(_Context.GetCustomersQuery(), OnLoadComplete, null);
            ...
            client.GetAlbumsCompleted += delegate(object sender, MaFamilleService.GetAlbumsCompletedEventArgs e)
            {
                ...
                if (e.Result != null)
                {
                    ...
                    foreach (MaFamilleService.AlbumModel album in albums)
                    {
                        ...
                        _album.EditClick = new DelegateCommand<object>(obj =>
                        {
                            if (obj != null)
                            {
                        ...
                        _album.Click = new DelegateCommand<object>(obj =>
                            {
                                if (obj != null)
                                {
                                    MyPhotos myPhotoView = new MyPhotos();
                                    PhotoViewModel myPhotoVM = new PhotoViewModel((obj as AlbumModel).AlbumName);
                                    myPhotoView.DataContext = myPhotoVM;
                                    IRegion contentRegion = RegionManager.Regions["ContentRegion"];
                                    contentRegion.Add(myPhotoView);
                                    contentRegion.Activate(myPhotoView);
                                }
                            });
