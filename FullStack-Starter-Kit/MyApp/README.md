...
```bash
dotnet run --project src/Host/MyApp.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    MyApp.Api/                    API composition root
    MyApp.AppHost/                .NET Aspire orchestrator
    MyApp.DbMigrator/             One-shot migrate / seed runner
    MyApp.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/MyApp.Api/appsettings.json`.
...
```bash
dotnet test src/MyApp.slnx       # integration tests require Docker
```
