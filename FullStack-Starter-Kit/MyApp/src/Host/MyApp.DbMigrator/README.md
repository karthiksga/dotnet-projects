...
dotnet run --project src/Host/MyApp.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/MyApp.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/MyApp.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/MyApp.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/MyApp.DbMigrator -- seed-demo
```
...
as `MyApp.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `MyApp.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/MyApp.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: MyApp.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/MyApp.AppHost` —
