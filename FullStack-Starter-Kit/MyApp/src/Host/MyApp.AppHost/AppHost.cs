...
// Per-app prefix from the AppHost assembly name (MyApp.AppHost -> fsh-starter); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var migrator = builder.AddProject<Projects.MyApp_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "MyApp.Migrations.PostgreSQL")
...
var demoSeeder = builder.AddProject<Projects.MyApp_DbMigrator>($"{appPrefix}-demo-seeder")
    .WithReference(postgres)
    .WaitFor(postgres)
...
// API Service
var api = builder.AddProject<Projects.MyApp_Api>($"{appPrefix}-api")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WaitFor(redis)
    ...
    .WithEnvironment("MailOptions__DisplayName", "MyApp")
