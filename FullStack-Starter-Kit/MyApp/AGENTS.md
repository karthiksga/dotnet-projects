...
| `src/Host/MyApp.Api` | Composition-root Web API host. |
| `src/Host/MyApp.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/MyApp.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/MyApp.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/MyApp.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/MyApp.slnx                   # build backend
dotnet run --project src/Host/MyApp.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/MyApp.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/MyApp.DbMigrator -- apply [--seed]
dotnet run --project src/Host/MyApp.DbMigrator -- list-pending
```
