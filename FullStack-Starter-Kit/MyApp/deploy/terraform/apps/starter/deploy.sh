#!/usr/bin/env bash
#
# One-command deploy of the MyApp Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/MyApp.Api/MyApp.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/MyApp.DbMigrator/MyApp.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
