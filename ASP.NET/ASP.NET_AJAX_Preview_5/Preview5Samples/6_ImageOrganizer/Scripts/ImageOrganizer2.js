...
var imagesList, detailPane, editMode = false, data = {},
...
function convertBackName(value, binding) {
    ...
    var contributorList = binding.get_templateContext().keys["contributorList"];
...
function onContributorItemRendered(sender, args) {
    var tc = args.get_itemContext()
    var optionElement = tc.keys["option"];

    optionElement.selected = (data.people[tc.index] == tc.parentContext.dataItem.Contributor);

    bindFirstAndLastNames(args.get_dataItem(), optionElement, "innerHTML");
}
...
function addNewImage() {
    ...
    imagesList.get_contexts()[newIndex].keys.listitem.scrollIntoView(false);
}
