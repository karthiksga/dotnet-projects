import { createDiagnosticCollector, getDoc, ignoreDiagnostics, typespecTypeToJson, validateDecoratorUniqueOnNode, } from "@typespec/compiler";
...
export const $cookie = (context, entity, cookieNameOrOptions) => {
 ...
 */
export function getCookieParamOptions(program, entity) {
 ...
 */
export function isCookieParam(program, entity) {
    return program.stateMap(HttpStateKeys.cookie).has(entity);
}
const [getQueryOptions, setQueryOptions] = useStateMap(HttpStateKeys.query);
...
const [getPathOptions, setPathOptions] = useStateMap(HttpStateKeys.path);
...
/** @internal */
export function resolvePathOptionsWithDefaults(options) {
    return {
        explode: options.explode ?? false,
 ...
 */
function isOriginalDecoratorApplication(context, entity) {
    const decoratorNode = context.decoratorTarget;
...
export const $patch = (context, entity, options) => {
 ...
 */
export function getPatchOptions(program, operation) {
