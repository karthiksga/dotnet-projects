import { createDiagnosticCollector, filterModelProperties, getDiscriminator, getEncode, getMediaTypeHint, isArrayModelType, navigateType, } from "@typespec/compiler";
 ...
 */
export var HttpPayloadDisposition;
...
export function resolveHttpPayload(program, type, visibility, disposition, options = {}) {
            ...
            case "bodyRoot":
                ...
                if (item.property.type.kind === "Union" && unionContainsFile(program, item.property.type)) {
                    reportDiagnostic(program, {
                        code: "http-file-structured",
                        messageId: "union",
                        target: item.property.node?.kind === SyntaxKind.ModelProperty
...
function unionContainsFile(program, u) {
    ...
    function unionContainsFileWorker(u, visited = new Set()) {
        if (visited.has(u))
            return false;
        visited.add(u);
        for (const { type } of u.variants.values()) {
            if (!!getHttpFileModel(program, type) ||
