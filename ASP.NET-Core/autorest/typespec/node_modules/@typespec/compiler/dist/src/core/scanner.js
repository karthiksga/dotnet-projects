...
/** @internal */
export var KeywordLimit;
(function (KeywordLimit) {
    KeywordLimit[KeywordLimit["MinLength"] = 2] = "MinLength";
    KeywordLimit[KeywordLimit["MaxLength"] = 12] = "MaxLength";
    ...
    function scanConflictMarker() {
        ...
        if (marker === CharCode.LessThan || marker === CharCode.GreaterThan) {
            ...
            while (position < endPosition) {
                const ch = input.charCodeAt(position);
                if ((ch === CharCode.Equals || ch === CharCode.GreaterThan) &&
                    ch !== marker &&
...
function skipMultiLineComment(input, position, endPosition = input.length) {
    position += 2; // consume '/*'
    for (; position < endPosition; position++) {
        if (input.charCodeAt(position) === CharCode.Asterisk &&
