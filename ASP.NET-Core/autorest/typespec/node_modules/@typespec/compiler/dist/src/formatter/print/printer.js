...
export function printAugmentDecorator(path, options, print) {
    const node = path.node;
    const target = node.target.kind === SyntaxKind.Identifier
        ? printIdentifier(node.target, "allow-reserved")
        : path.call(print, "target");
...
export function printModelExpression(path, options, print) {
    ...
    if (inBlock) {
        ...
        const properties = indent(joinMembersInBlock(path, "properties", options, print, ifBreak(",", ", "), softline));
...
function printHeritageClause(path, print, keyword, propertyName) {
    const ref = path.node[propertyName];
    if (!ref) {
        return "";
    }
    const printed = [`${keyword} `, path.call(print, propertyName)];
    if (isMultiArgTemplateReference(ref)) {
        return [" ", printed];
    }
    return group(indent([ifBreak(line, " "), printed]));
}
function isMultiArgTemplateReference(node) {
    return (node.kind === SyntaxKind.TypeReference && node.arguments.length >= 2);
}
...
function isInMultiTemplateArgumentList(path) {
