...
import { IdentifierKind, ModifierFlags, ResolutionResultFlags, SymbolFlags, SyntaxKind, } from "./types.js";
var CheckFlags;
(function (CheckFlags) {
    /** No flags set. */
...
export function createChecker(program, resolver) {
    ...
    function checkDeclaredTypeOrIndeterminate(ctx, sym, node) {
        const type = sym.flags & SymbolFlags.Model
    ...
    function getReferencedModel(propertyNode) {
        ...
        function getReferencedTypeFromTemplateDeclaration(node) {
            ...
            if (node?.parent?.kind === SyntaxKind.TemplateArgument &&
            ...
            else if (isModelOrArrayType(node) &&
                templateParmaeterDeclNode.constraint.kind !== SyntaxKind.ValueOfExpression) {
    ...
    function checkSymbolAccess(sourceLocation, node, symbol) {
        if (!symbol)
            return;
        const isInternalDeclaration = (symbol.flags & (SymbolFlags.Internal | SymbolFlags.Declaration)) ===
    ...
    function checkFunctionCallArguments(ctx, args, target, diagnosticTarget) {
        let satisfied = true;
        ...
        for (const param of target.parameters) {
            if (param.rest) {
                ...
                for (const [idx, restArg] of restArgs.entries()) {
                    const resolved = collector.pipe(checkEntityAssignableToConstraint(restArg, constraint, restArgExpressions[idx]));
                    satisfied &&= !!resolved;
    ...
    function defineProperty(properties, newProp, diagnosticTarget) {
        if (properties.has(newProp.name)) {
            ...
            reportCheckerDiagnostic(modelName
                ? createDiagnostic({
    ...
    function lateBindMemberContainer(type) {
        ...
        switch (type.kind) {
            case "Model":
                ...
                break;
            case "Interface":
                type.symbol = createSymbol(type.node, type.name, SymbolFlags.Interface | SymbolFlags.LateBound);
                mutate(type.symbol).type = type;
                break;
            case "Union":
                if (!type.name)
    ...
    function lateBindMembers(type) {
        ...
        function lateBindMember(member, kind) {
            ...
            const sym = createSymbol(member.node, member.name, kind | SymbolFlags.LateBound, containerSym);
    ...
    function checkAugmentDecorator(ctx, node) {
        ...
        if (links.isTemplateInstantiation) {
            ...
            program.reportDiagnostic(createDiagnostic({
                code: "augment-decorator-target",
                messageId: links.finalSymbol.flags & SymbolFlags.Model
                    ? "noModelExpression"
     ...
     */
    function isUsingBeforeFileNamespace(identifier) {
