...
import { defaultInternalTemplateSource, UriTemplateSource, } from "./template-source/index.js";
...
export async function initTypeSpecProjectWorker(host, directory, options = {}) {
    ...
    const source = options.templatesUrl !== undefined
    ...
    if (skipPrompts && !options.template) {
        ...
        throw new Error(`A template must be specified when --no-prompt is used. Specify one of the following templates via --template: ${Object.keys(index.templates)
            .map((t) => `"${t}"`)
            .join(", ")}`);
    }
    ...
    const template = index.templates[templateName];
...
async function loadTemplateIndex(source, url) {
    try {
        ...
        const code = e instanceof SyntaxError ? "init-template-invalid-json" : "init-template-download-failed";
...
/** @internal */
export function isTemplateCompatibleWithTspVersion(template) {
...
async function validateTemplate(template, index) {
