...
export function createLinter(program, loadLibrary, codeResolver) {
    const tracer = program.tracer.sub("linter");
    const resolveCode = (ref) => (codeResolver ? codeResolver.resolveCode(ref) : ref);
    /**
     ...
     */
    const reportIfAmbiguous = (ref, diagnostics) => {
    ...
    async function extendRuleSet(ruleSet) {
        ...
        if (ruleSet.extends) {
            for (const extendingRuleSetName of ruleSet.extends) {
                ...
                const ref = diagnostics.pipe(parseRuleReference(resolveCode(extendingRuleSetName)));
                if (ref) {
    ...
    async function registerLinterLibrary(name, lib) {
    ...
    function resolveRuleOptions(rule, enableValue) {
        const options = enableValue === true
            ? (rule.defaultOptions ?? {})
...
function createJSONSchemaValidatorForRuleOptions(schema) {
    return createJSONSchemaValidator(schema, { strict: false });
}
export function createLinterRuleContext(program, rule, options, diagnosticCollector) {
    return {
        program,
        options,
        reportDiagnostic,
...
export function createBuiltInLinterLibrary() {
    const builtInLinter = resolveLinterDefinition(builtInLinterLibraryName, createBuiltInLinter());
    return { linter: builtInLinter };
}
function createBuiltInLinter() {
