...
export interface DecoratorFunction {
    (context: DecoratorContext, target: any, ...customArgs: any[]): DecoratorValidatorCallbacks | void;
    namespace?: string;
}
...
export type Type = BooleanLiteral | Decorator | FunctionType | Enum | EnumMember | FunctionParameter | Interface | IntrinsicType | Model | ModelProperty | Namespace | NumericLiteral | Operation | Scalar | ScalarConstructor | StringLiteral | StringTemplate | StringTemplateSpan | TemplateParameter | TemplateParameterAccess | Tuple | Union | UnionVariant;
...
export type Value = ScalarValue | NumericValue | StringValue | BooleanValue | ObjectValue | ArrayValue | EnumValue | NullValue | FunctionValue;
interface BaseValue {
 ...
 * ```
 */
export interface FunctionValue<Parameters extends unknown[] = never[], ReturnType = unknown> extends BaseValue {
    valueKind: "Function";
...
export declare enum ResolutionResultFlags {
    None = 0,
    Resolved = 2,
    Unknown = 4,
    Ambiguous = 8,
 ...
 */
export type MemberContainerNode = ModelStatementNode | ModelExpressionNode | InterfaceStatementNode | EnumStatementNode | UnionStatementNode | IntersectionExpressionNode | ScalarStatementNode;
...
interface LinterRuleDefinitionBase<N extends string, DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> {
...
interface LinterRuleDefinitionSync<N extends string, DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> extends LinterRuleDefinitionBase<N, DM, Options> {
    /** Whether this is an async rule. Default is false */
    async?: false;
    /** Creator */
    create(context: LinterRuleContext<DM, Options>): SemanticNodeListener & {
        exit?: (context: Program) => void | undefined;
    };
}
interface LinterRuleDefinitionAsync<N extends string, DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> extends LinterRuleDefinitionBase<N, DM, Options> {
    /** Whether this is an async rule. Default is false */
    async: true;
    /** Creator */
    create(context: LinterRuleContext<DM, Options>): SemanticNodeListener & {
...
export type LinterRuleDefinition<N extends string, DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> = LinterRuleDefinitionSync<N, DM, Options> | LinterRuleDefinitionAsync<N, DM, Options>;
/** Resolved instance of a linter rule that will run. */
export type LinterRule<N extends string, DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> = LinterRuleDefinition<N, DM, Options> & {
...
export interface LinterRuleContext<DM extends DiagnosticMessages, Options extends Record<string, unknown> = Record<string, never>> {
...
export interface InvocationContext {
     ...
     */
    callDecorator<T extends Type, A extends any[], R>(decorator: (context: DecoratorContext, target: T, ...args: A) => R, target: T, ...args: A): R;
    /**
     ...
     */
    callFunction<A extends any[], R>(func: (context: FunctionContext, ...args: A) => R, ...args: A): R;
}
/**
