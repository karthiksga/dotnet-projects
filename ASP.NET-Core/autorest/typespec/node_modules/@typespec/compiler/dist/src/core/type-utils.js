...
export function isArrayModelType(programOrType, maybeType) {
    const type = maybeType ?? programOrType;
    return Boolean(type.indexer && type.indexer.key.name === "integer");
}
export function isRecordModelType(programOrType, maybeType) {
    const type = maybeType ?? programOrType;
...
export function getFullyQualifiedSymbolName(sym, options) {
    if (!sym)
        return "";
    if (sym.symbolSource)
        sym = sym.symbolSource;
    const parent = sym.parent && !(sym.parent.flags & SymbolFlags.SourceFile) ? sym.parent : undefined;
