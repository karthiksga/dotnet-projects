...
export function resolveYamlPositionDetail(document, position, log) {
    ...
    if (isWhitespaceStringOrUndefined(targetLine) || isCommentLine(targetLine)) {
        ...
        if (indent === 0) {
            ...
            if (isMap(yamlDoc.contents)) {
                rootProperties.push(...yamlDoc.contents.items.map((item) => item.key.source ?? ""));
            }
            else if (isScalar(yamlDoc.contents) &&
        ...
        for (let i = position.line - 1; i >= 0; i--) {
            ...
            if (preIndent === indent) {
                ...
                const found = findScalarNode(yamlDoc, document.offsetAt({
                    line: i,
                    character: preIndent,
                }));
                if (!found) {
                ...
                if (isPair(last) &&
                    ...
                    return {
                        ...
                        siblings: isMap(last.value)
                            ? (last.value?.items.map((item) => item.key.source ?? "") ?? [])
                            : [],
                        yamlDoc,
                        sourceRange: yp.sourceRange,
                        cursorPosition: pos,
...
function createYamlPathFromVisitScalarNode(info, offset, log, yamlDoc) {
    ...
    if (isDocument(last)) {
        ...
        if (key === "value" && newline && newline.offset < offset) {
            ...
            const targetSiblings = isMap(parent)
                ? parent.items.filter((item) => item !== last).map((item) => item.key.source ?? "")
                : [];
            return {
                path: path,
...
function getIndentOfYamlLine(line) {
    return line.search(/[^\s-]/);
}
function findScalarNode(document, pos) {
    let found = undefined;
    visit(document, {
        Node: (key, n, path) => {
            if (isScalar(n)) {
                const [start, endValue, _endNode] = n.range ?? [];
                if (start === undefined || endValue === undefined)
                    return undefined;
...
//# sourceMappingURL=yaml-resolver.js.map