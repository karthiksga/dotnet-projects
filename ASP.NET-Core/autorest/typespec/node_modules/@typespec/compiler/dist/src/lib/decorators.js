import { getPropertyType, validateDecoratorNotOnType, validateDecoratorUniqueOnNode, } from "../core/decorator-utils.js";
...
import { getDocDataInternal, getMaxItemsAsNumeric, getMaxLengthAsNumeric, getMaxValueAsNumeric, getMaxValueExclusiveAsNumeric, getMinItemsAsNumeric, getMinLengthAsNumeric, getMinValueAsNumeric, getMinValueExclusiveAsNumeric, setDiscriminatedOptions, setDiscriminator, setDocData, setMaxItems, setMaxLength, setMaxValue, setMaxValueExclusive, setMinItems, setMinLength, setMinValue, setMinValueExclusive, } from "../core/intrinsic-type-state.js";
...
export { getPagingOperation, isList } from "./paging.js";
...
export function isDateTimeType(program, target) {
 ...
 */
function validateTargetingComparableType(context, target, decoratorName) {
    const valid = isTypeIn(getPropertyType(target), (x) => isNumericType(context.program, x) || isDateTimeType(context.program, x));
    if (!valid) {
 ...
 */
function validateValueAssignableToTarget(context, target, value) {
    const targetType = getPropertyType(target);
    if (isNumeric(value)) {
        value = $(context.program).value.createNumeric(value);
    }
    const [assignable, diagnostics] = $(context.program).value.isOfType.withDiagnostics(value, targetType, context.getArgumentTarget(0));
    if (!assignable) {
 ...
 */
function getTypeForArrayValidation(target) {
    if (target.kind === "ModelProperty") {
        return target.type;
    }
    else {
        return target.kind === "Model" ? target : target.type;
    }
}
/**
 * Validate the given target is an array type or a union containing at least an array type.
 */
function validateTargetingAnArray(context, target, decoratorName) {
...
const [_getMediaTypeHint, _setMediaTypeHint] = useStateMap(createStateSymbol("mediaTypeHint"));
export const $mediaTypeHint = (context, target, mediaType) => {
 ...
 */
export function setMediaTypeHint(program, target, mediaType) {
    const mimeTypeObj = parseMimeType(mediaType);
    compilerAssert(mimeTypeObj !== undefined, `Invalid MIME type '${mediaType}' provided to setMediaTypeHint`);
 ...
 */
export function getMediaTypeHint(program, target) {
    switch (target.kind) {
        case "Scalar":
            ...
            // itself.
            const isTypeSpecString = target.name === "string" &&
        ...
        case "Model": {
            ...
            if (!hint) {
                ...
                const ancestor = target.kind === "Model"
                    ? target.baseModel
                    : target.kind === "Scalar"
                        ? target.baseScalar
                        : undefined;
...
const [getFriendlyName, setFriendlyName] = useStateMap(createStateSymbol("friendlyNames"));
...
const [getOverloads, setOverloads] = useStateMap(createStateSymbol("overloadedByKey"));
const [getOverloadedOperation, setOverloadBase] = useStateMap(createStateSymbol("overloadsOperation"));
/**
...
function areOperationsInSameContainer(op1, op2) {
    ...
    return (op1.namespace === op2.namespace ||
...
export const discriminatedDecorator = (context, entity, options = {}) => {
...
const [getExamplesState, setExamples] = useStateMap(createStateSymbol("examples"));
...
const [getOpExamplesState, setOpExamples] = useStateMap(createStateSymbol("opExamples"));
