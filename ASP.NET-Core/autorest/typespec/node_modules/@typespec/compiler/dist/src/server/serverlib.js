import { CodeActionKind, CompletionList, DiagnosticSeverity, DiagnosticTag, DocumentHighlightKind, FileChangeType, FoldingRangeKind, MarkupKind, Range, SemanticTokensBuilder, TextDocumentSyncKind, TextEdit, } from "vscode-languageserver";
...
export function createServer(host, clientConfigsProvider) {
    ...
    async function getFoldingRanges(params) {
        ...
        return ranges;
        function addRange(startPos, endPos, kind) {
    ...
    async function resolveCodeAction(codeAction) {
    ...
    function convertCodeFixEdits(edits) {
    ...
    function convertCodeFixEdit(edit) {
        switch (edit.kind) {
            case "insert-text":
    ...
    function getLocations(targets) {
        return targets?.map(getLocation).filter((x) => !!x) ?? [];
    }
        ...
        switch (n.kind) {
            ...
            case SyntaxKind.TypeReference:
                ...
                if (position === n.end) {
                    ...
                    const closeChar = n.kind === SyntaxKind.TypeReference ? CharCode.GreaterThan : CharCode.CloseParen;
