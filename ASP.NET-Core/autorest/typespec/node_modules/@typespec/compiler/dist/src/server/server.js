...
function main() {
    ...
    console.log = (data, ...args) => connection.console.info(format(data, ...args));
    // eslint-disable-next-line no-console
    console.info = (data, ...args) => connection.console.info(format(data, ...args));
    // eslint-disable-next-line no-console
    console.debug = (data, ...args) => connection.console.debug(format(data, ...args));
    // eslint-disable-next-line no-console
    console.warn = (data, ...args) => connection.console.warn(format(data, ...args));
    // eslint-disable-next-line no-console
    console.error = (data, ...args) => connection.console.error(format(data, ...args));
    const host = {
        ...
        log(log) {
            ...
            switch (log.level) {
                ...
                case "error":
                    ...
                    break;
                default:
                    connection.console.error(`Log Message with invalid LogLevel (${log.level}). Raw Message: ${fullMessage}`);
                    break;
            }
        },
    ...
    const validateInitProjectTemplate = "typespec/validateInitProjectTemplate";
    connection.onRequest(validateInitProjectTemplate, profile(s.validateInitProjectTemplate));
    const getInitProjectContextRequestName = "typespec/getInitProjectContext";
    connection.onRequest(getInitProjectContextRequestName, profile(s.getInitProjectContext));
    const initProjectRequestName = "typespec/initProject";
    connection.onRequest(initProjectRequestName, profile(s.initProject));
    const compileProjectRequestName = "typespec/internalCompile";
    ...
    globalThis.lspConnection = connection;
