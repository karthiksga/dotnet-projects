...
export function createSuppressionTracker(sourceResolution, codeResolver) {
    ...
    return {
        ...
        getUnusedSuppressions() {
            ...
            for (const suppression of suppressions.values()) {
                ...
                const availability = getSuppressionSourceAvailability(resolveCode(codeResolver, suppression.directive.code), sourceResolution);
...
export function findDuplicateSuppressions(sourceResolution) {
    const duplicates = [];
 ...
 */
export function findAmbiguousSuppressions(sourceResolution, codeResolver) {
    const ambiguous = [];
...
function collectSuppressions(sourceResolution) {
...
function resolveCode(codeResolver, code) {
    return codeResolver ? codeResolver.resolveCode(code) : code;
}
function getSuppressionSourceAvailability(code, sourceResolution) {
    if (compilerDiagnosticCodes.has(code) ||
...
function matchesDiagnosticSource(code, source) {
    return code.startsWith(`${source}/`);
}
export function findDirectiveSuppressingOnNode(code, node, codeResolver) {
    let current = node;
    do {
 ...
 */
export function findDirectiveSuppressingCode(code, directives, codeResolver) {
...
export function createRemoveUnusedSuppressionCodeFix(node) {
...
//# sourceMappingURL=suppression-tracking.js.map