 ...
 */
setOperationId,] = useStateMap(createStateSymbol("operationIds"));
/**
...
export const tagMetadataDecorator = (context, entity, name, tagMetadata) => {
    ...
    if (typeof name !== "string") {
        ...
        // Validate the additionalInfo model
        if (!validateAdditionalInfoModel(context.program, context.getArgumentTarget(1), resolvedMetadata, "TypeSpec.OpenAPI.TagMetadata")) {
            return;
        }
        // Validate the externalDocs.url property
        if (resolvedMetadata.externalDocs?.url) {
            if (!validateIsUri(context.program, context.getArgumentTarget(1), resolvedMetadata.externalDocs.url, "externalDocs.url")) {
                return;
            }
        }
