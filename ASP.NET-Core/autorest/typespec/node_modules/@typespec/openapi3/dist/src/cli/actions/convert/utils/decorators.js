...
function getPagingLinkDecorators(schema) {
    const decorators = [];
    ...
    for (const [extensionKey, decoratorName] of Object.entries(linkExtensions)) {
        const extensionValue = schema[extensionKey];
...
export function getParameterDecorators(parameter, context) {
    ...
    const xMsListOffset = parameter["x-ms-list-offset"];
    ...
    const xmsListContinuationToken = parameter["x-ms-list-continuation-token"];
    ...
    const xmsListPageIndex = parameter["x-ms-list-page-index"];
...
function getArrayEncodingDecorator(parameter) {
...
function createTSValueFromObjectValue(value) {
...
export function getDecoratorsForSchema(schema, context) {
    ...
    const xmsListContinuationToken = schema["x-ms-list-continuation-token"];
    if (xmsListContinuationToken === true) {
        decorators.push({
            name: "continuationToken",
            args: [],
...
function getDurationSchemaDecorators(schema, effectiveType) {
    const decorators = [];
    // Get the x-ms-duration value (seconds or milliseconds)
    const xmsDuration = schema["x-ms-duration"];
