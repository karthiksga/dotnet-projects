import { ArrayBuilder, createAssetEmitter, ObjectBuilder, setProperty, } from "@typespec/asset-emitter";
import { compilerAssert, getDiscriminatedUnion, getDoc, getExamples, getMaxValueExclusive, getMinValueExclusive, getSummary, } from "@typespec/compiler";
...
export class OpenAPI31SchemaEmitter extends OpenAPI3SchemaEmitterBase {
    #applySchemaExamples(program, type, target) {
        ...
        if (examples.length > 0) {
            setProperty(target, "examples", examples.map((example) => serializeExample(program, example.value, type)));
        }
    }
    ...
    #annotatedEnumSchema(en) {
        const program = this.emitter.getProgram();
        const oneOf = [];
        for (const member of en.members.values()) {
            const value = member.value ?? member.name;
            const subschema = { const: value };
    ...
    combineRefWithConstraints(refSchema, constraints) {
        ...
        const merged = new ObjectBuilder(refSchema);
