...
export function resolveOptions(context) {
    ...
    const fileTypes = Array.isArray(rawFileType)
        ? rawFileType
        : [rawFileType ?? findFileTypeFromFilename(resolvedOptions["output-file"])];
    const outputFile = resolvedOptions["output-file"] ??
        (fileTypes.length > 1
    ...
    const enumStrategy = resolvedOptions["enum-strategy"] ?? "default";
...
function createOAPIEmitter(context, options, specVersion = "3.0.0") {
                ...
                if (serviceRecord.versioned) {
                    for (const documentRecord of serviceRecord.versions) {
                        await emitFile(program, {
                            path: resolveOutputFile(serviceRecord.service, multipleService, fileType, documentRecord.version),
    ...
    function resolveOutputFile(service, multipleService, fileType, version) {
    ...
    function getOpenAPIParameterBase(param, visibility) {
        ...
        let schema = applyEncoding(program, param, applyIntrinsicDecorators(param, typeSchema), options);
    ...
    function emitSchemas(serviceNamespace) {
        ...
        function processUnreferencedSchemas() {
            const authSchemeModels = new Set(serviceAuth.schemes.map((s) => s.model));
    ...
    function resolveDocumentTags(service) {
        ...
        for (const tag of metadataList ?? []) {
            const { name, ...rest } = tag;
            const tagData = { name, ...rest };
            // For OpenAPI 3.0 and 3.1, convert 'parent', 'summary', and 'kind' to x-oai- prefixed extensions
            if (specVersion !== "3.2.0") {
                if (tag.parent) {
                    tagData["x-oai-parent"] = tag.parent;
                    delete tagData.parent;
                }
                if (tag.summary) {
                    tagData["x-oai-summary"] = tag.summary;
                    delete tagData.summary;
                }
                if (tag.kind) {
                    tagData["x-oai-kind"] = tag.kind;
                    delete tagData.kind;
                }
            }
            tags.push(tagData);
