...
export class OpenAPI3SchemaEmitterBase extends TypeEmitter {
    ...
    modelDeclaration(model, _) {
        ...
        if (model.baseModel) {
            setProperty(schema, "allOf", Builders.array([this.emitter.emitTypeReference(model.baseModel)]));
        }
     ...
     */
    combineRefWithConstraints(refSchema, constraints) {
        return {
            allOf: [refSchema],
            ...constraints,
        };
     ...
     */
    #unionVariantEnvelopeVisibilityMap = new WeakMap();
    /**
     ...
     */
    #getOrCreateVariantEnvelopeModel(union, variantName, variant) {
        ...
        if (!entry) {
            ...
            if (usage)
                this._visibilityUsage.manuallyTrack(entry.default, usage);
        }
        const visibility = this.#getVisibilityContext();
        ...
        if (this._metadataInfo.isTransformed(variant, visibility)) {
            ...
            if (!byVis) {
                ...
                // Manually track the model's usage according to the union's usage.
                if (usage)
                    this._visibilityUsage.manuallyTrack(byVis, usage);
    ...
    discriminatedUnion(union) {
        ...
        if (union.options.envelope === "none") {
            ...
            for (const [variantName, variant] of union.variants) {
                envelopeVariants.set(variantName, this.#getOrCreateVariantEnvelopeModel(union, variantName, variant));
            }
            const items = new ArrayBuilder();
    ...
    #addDefaultVariantToMapping(union, mapping) {
        ...
        const discriminatorProp = union.defaultVariant.properties.get(union.options.discriminatorPropertyName);
        if (discriminatorProp) {
            ...
            if (discriminatorValue) {
                ...
                mapping[discriminatorValue] = ref.value.$ref;
            }
        }
    }
    #getStringValueFromType(type) {
        if (type.kind === "String") {
            return type.value;
        }
